...
export default function SlideImageMobile({ dir, slide, currentLang, activeSlide, index }) {
  return (
    <>
      {dir === 'rtl' ? (
        <m.div className="absolute inset-0 overflow-hidden bg-neutral-900 ">
          {slide?.translations[currentLang.value].background ? (
            <m.img
              src={`${URL_IMAGE}/${slide.translations[currentLang.value].background}`}
              ...
              animate={{
                ...
                clipPath:
                  activeSlide === index ? 'circle(100% at 35% 50%)' : 'circle(100% at 35% 50%)',
          ...
          {slide?.translations[currentLang.value].background ? (
            <m.img
              ...
              animate={{
                y: activeSlide === index ? '0%' : '100%',
                clipPath:
                  activeSlide === index ? 'circle(100% at 65% 50%)' : 'circle(100% at 65% 50%)',
                scale: activeSlide === index ? 1 : 0.6,
              }}
              transition={{
                y: { duration: 1.2, ease: 'easeOut' },
