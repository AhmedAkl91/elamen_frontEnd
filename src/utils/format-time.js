...
export function scheduleDate(date, format) {
  if (!date) {
    return null;
  }
  const isValid = dayjs(date).isValid();
...
export function fDate2(date, format) {
  if (!date) {
    return null;
  }
  ...
  return isValid ? dayjs(date).format(format ?? formatStr.split.data2) : 'Invalid time value';
}
